...
! ------------------------------------------------------------------------------
   ...
   use mod_constants, only: grav, alpha0, rho0, epsilp, epsilz, &
                            onem, onecm, onemm
   use mod_xc
   use mod_vcoord,    only: vcoord_tag, vcoord_isopyc_bulkml
   use mod_grid,      only: scuxi, scvyi
   use mod_eos,       only: rho, sig0, p_alpha
   ...
   ! ---------------------------------------------------------------------------
      ...
      do j = - 1, jj + 2
         ...
         do i = max(- 1, ifp(j,l)), min(ii + 2, ilp(j,l))
            ...
            bfsqi(i,j,1) = .5_r8*grav*grav &
                           *( rho(p(i,j,2), temp(i,j,2+nn), saln(i,j,2+nn)) &
                            - rho(p(i,j,2), temp(i,j,1+nn), saln(i,j,1+nn))) &
            ...
            if (kfpl > kk) then
               ...
               ! discretization.
               k = kfpl - 1
               ctd(k) = - 2._r8*sls2(k  )/(delp(k)*(delp(k  ) + delp(k+1)))
               btd(k) = 1._r8 - ctd(k)
               rtd(k) = bfsq(k)
               do k = kfpl, kk - 1
                  atd(k) = - 2._r8*sls2(k-1)/(delp(k)*(delp(k-1) + delp(k  )))
                  ctd(k) = - 2._r8*sls2(k  )/(delp(k)*(delp(k  ) + delp(k+1)))
   ...
   subroutine cmnfld_mldl82(m, n, mm, nn, k1m, k1n)
   ! ---------------------------------------------------------------------------
   ! Estimate mixed layer depth using the method by Levitus (1982).
   ! ---------------------------------------------------------------------------
              ...
              do
                 if (dp(i,j,km) > onecm) then
                    ...
                    if (dblo <= dbcl82) then
                       ...
                       dbup = min(dbup, dbcl82 - epsilp)
                       mldl82(i,j) = ( zup*(dblo - dbcl82) &
                                     + zlo*(dbcl82 - dbup))/(dblo - dbup) &
                                   - z(i,j,1)
                       dpmll82(i,j) = ( pup*(dblo - dbcl82) &
                                      + plo*(dbcl82 - dbup))/(dblo - dbup) &
                                    - p(i,j,1)
                      exit
                    endif
                 endif
                 k = k + 1
                 if (k > kk) then
                    mldl82(i,j) = z(i,j,kk+1) - z(i,j,1)
                    dpmll82(i,j) = p(i,j,kk+1) - p(i,j,1)
                    exit
                 endif
                 km = k + mm
               enddo
      ...
      if (csdiag) then
         if (mnproc == 1) then
           write (lp,*) 'cmnfld_mldl82:'
         endif
         call chksum(mldl82, 1, halo_ps, 'mldl82')
         call chksum(dpmll82, 1, halo_ps, 'dpmll82')
      endif

   end subroutine cmnfld_mldl82

   subroutine cmnfld_mldb04(m, n, mm, nn, k1m, k1n)
   ! ---------------------------------------------------------------------------
   ! Estimate mixed layer depth using the method by de Boyer Montégut et al.
   ! (2004).
   ! ---------------------------------------------------------------------------
      ...
      do j = 1, jj
         do l = 1, isp(j)
            do i = max(1, ifp(j,l)), min(ii, ilp(j,l))
              ...
              if (z(i,j,kk+1) - .5_r8*dz(i,j,kk) - z(i,j,1) < zrefb04) then
                 mldb04(i,j) = z(i,j,kk+1) - z(i,j,1)
                 dpmlb04(i,j) = p(i,j,kk+1) - p(i,j,1)
                 exit
              endif

              k = 1
              do
                 dzlo = z(i,j,k+1) - .5_r8*dz(i,j,k) - z(i,j,1)
                 if (dzlo >= zrefb04) exit
                 dzup = dzlo
                 k = k + 1
              enddo
              km = k + mm
              ...
              if (k == 1) then
                 sig0ref = sig0(temp(i,j,k1m), saln(i,j,k1m))
              elseif (abs(dzlo - dzup) < epsilz) then
                 sig0ref = sig0(temp(i,j,km), saln(i,j,km))
              else
                 sig0ref = &
                    ( sig0(temp(i,j,km-1), saln(i,j,km-1))*(dzlo - zrefb04) &
                    + sig0(temp(i,j,km  ), saln(i,j,km  ))*(zrefb04 - dzup)) &
                    /(dzlo - dzup)
              endif
              ...
              do
                 if (dp(i,j,km) > onecm) then
                    plo = p(i,j,k) + .5_r8*dp(i,j,km)
                    zlo = z(i,j,k) + .5_r8*dz(i,j,k )
                    dslo = sig0(temp(i,j,km), saln(i,j,km)) - sig0ref
                    if (dslo <= drcb04) then
                       pup = plo
                       zup = zlo
                       dsup = dslo
                    else
                       mldb04(i,j) = ( zup*(dslo - drcb04) &
                                     + zlo*(drcb04 - dsup))/(dslo - dsup) &
                                   - z(i,j,1)
                       dpmlb04(i,j) = ( pup*(dslo - drcb04) &
                                      + plo*(drcb04 - dsup))/(dslo - dsup) &
                                    - p(i,j,1)
                      exit
                    endif
                 endif
      ...
      if (csdiag) then
         if (mnproc == 1) then
           write (lp,*) 'cmnfld_mldb04:'
         endif
         call chksum(mldb04, 1, halo_ps, 'mldb04')
         call chksum(dpmlb04, 1, halo_ps, 'dpmlb04')
      endif

   end subroutine cmnfld_mldb04
   ...
   ! ---------------------------------------------------------------------------
      ...
      if ((vcoord_tag /= vcoord_isopyc_bulkml .and. mldmth == 'lev82') .or. &
          sum( ACC_MLDL82  (1:nphy) + ACC_MLDL82MN(1:nphy) &
             + ACC_MLDL82MX(1:nphy) + ACC_MLDL82SQ(1:nphy)) /= 0) then
         call cmnfld_mldl82(m, n, mm, nn, k1m, k1n)
      endif

      ! Estimate mixed layer depth using the method by de Boyer Montégut et al.
      ! (2004).

      if ((vcoord_tag /= vcoord_isopyc_bulkml .and. mldmth == 'boy04') .or. &
          sum( ACC_MLDB04  (1:nphy) + ACC_MLDB04MN(1:nphy) &
             + ACC_MLDB04MX(1:nphy) + ACC_MLDB04SQ(1:nphy)) /= 0) then
         call cmnfld_mldb04(m, n, mm, nn, k1m, k1n)
      endif

      if (vcoord_tag /= vcoord_isopyc_bulkml) then
         if     (mldmth == 'lev82') then
            !$omp parallel do private(l, i)
            do j = 1, jj
               do l = 1, isp(j)
                  do i = max(1, ifp(j,l)), min(ii, ilp(j,l))
                     mld(i,j) = mldl82(i,j)
                     dpml(i,j) = dpmll82(i,j)
                  enddo
               enddo
            enddo
            !$omp end parallel do
         elseif (mldmth == 'boy04') then
            !$omp parallel do private(l, i)
            do j = 1, jj
               do l = 1, isp(j)
                  do i = max(1, ifp(j,l)), min(ii, ilp(j,l))
                     mld(i,j) = mldb04(i,j)
                     dpml(i,j) = dpmlb04(i,j)
                  enddo
               enddo
            enddo
            !$omp end parallel do
            ...
            if (mnproc == 1) then
               write (lp,'(3a)') ' mldmth = ',trim(mldmth),' is unsupported!'
            endif
            call xcstop('(cmnfld1)')
            stop '(cmnfld1)' 
         endif
